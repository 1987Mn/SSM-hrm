...
<table width='100%' align='center' style='font-size: 13px;'
	...
	<tr>
		<td
			...
			总页数:${requestScope.pageModel.pages}&nbsp;&nbsp;总记录条数:${requestScope.pageModel.total}
			<c:choose>
				<c:when test="${requestScope.pageModel.pages==0}">
					<span class='disabled'>上一页</span>
					<span class='disabled' >下一页</span>
				</c:when>

				<c:when test="${requestScope.pageModel.pages==1}">
					<span class='disabled'>上一页</span>
					<c:forEach begin="1" end="${requestScope.pageModel.pages}" var="i">
						<c:choose>
							<c:when test="${requestScope.pageModel.pageNum==i}">
				...
				<c:when test="${requestScope.pageModel.pageNum==1}">
					<span class='disabled'>上一页</span>
					<c:forEach begin="1" end="${requestScope.pageModel.pages}" var="i">
						<c:choose>
							<c:when test="${requestScope.requestScope.pageModel.pageNum==i}">
								<span class='current'>${i}</span>
							</c:when>
							<c:otherwise>
								<a href='#' onclick="toPage(${i})">${i}</a>
					...
					<span class='current'><a href='#'
						onclick="toPage(${requestScope.pageModel.pageNum+1})">下一页</a></span>
				</c:when>

				<c:when test="${requestScope.pageModel.pageNum==requestScope.pageModel.pages}">
					<span class='current'><a href='#' onclick="toPage(${requestScope.pageModel.pageNum-1})">上一页</a></span>
					<c:forEach begin="1" end="${requestScope.pageModel.pages}" var="i">
						<c:choose>
							<c:when test="${requestScope.pageModel.pageNum==i}">
								<span class='current'>${i}</span>
